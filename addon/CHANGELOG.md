...
- Added LAN-first control for RGBIC segments and hardware DreamView, with an explicit
  opt-in IoT or Platform cloud fallback after LAN status probes are exhausted.
- Added robust Govee two-factor authentication with one-shot codes and a configurable app version.
- Added native fan entities, air-quality sensors, battery/Wi-Fi diagnostics, and additional device quirks.
- Added categorized scene catalogs with icons and hints, active-scene tracking, and next/previous controls.
- Added IoT work-mode freshness, Web UI status badges and room grouping, and experimental LAN music palettes.
- Added an enabled-by-default Web UI switch that hides devices with no responsive LAN or cloud transport.
- Added a dedicated Web UI switch for the `dreamViewToggle` capability; it uses
  the same LAN-first routing and explicitly configured cloud fallback as Home Assistant.

### Fixed

- Keep segment entities available from local quirks during offline startup, while gating video
  DreamView switches by the sync centers selected in the Govee home-layout topology.
- Generate a unique UUID for every Platform API request and a collision-resistant IoT transaction ID.
- Bound LAN status retries and suspend background polling of repeatedly unresponsive devices.
- Preserve active animated scenes until an explicit color command or power-off changes the mode.
- Preserve the timezone supplied automatically by Home Assistant Supervisor, so console, file,
  API, and Web UI logs use the system timezone without extra add-on configuration.
- Report cloud reachability independently from the newest LAN/IoT device state, verify it with
  non-blocking Platform probes, and ignore stale cached account online flags.
- Derive BaseGroup and DreamView scene power from their member topology: a group is on only when
  every known member is on, and member changes refresh the corresponding Home Assistant entity.
- Separate groups and DreamView scenes from physical-device rooms and counters in the Web UI, and
  stop rendering unsupported brightness, color, and scene controls for those virtual objects.
- Add saved Music DreamView cards from the home-layout API to the Web UI group/scene block, retain
  their API-derived member counts, and route them through the selected music center.
- Render unknown/off Web UI switches with the neutral off appearance instead of Bootstrap's blue
  indeterminate fill.
- Route scenes dynamically: verified LAN control first, Platform automatically for devices or
  individual scenes without a local path, and decoded IoT packets as the final transport.
- Correct H60B0 device classification and retain the expanded upstream device-quirk fixes.

### Changed

- Merge and cache scene metadata from undocumented, Platform API, and local database sources.
- Give the segment/DreamView fallback setting a readable UI name and explain every transport choice.
- Stop exposing the Platform API subscription topic in Govee Push logs.
- Keep this work unreleased until a new tag and GitHub Release are explicitly published.

## [2026-08-24] - 2026-08-24

### Fixed

- Restored distinct Home Assistant images for every architecture declared by the add-on.
- Pinned each add-on build to the standalone image from the same release tag.
- Removed duplicated architecture and repository values from the release workflow.

### Changed

- Verification, multi-architecture builds, signing, and publication now run automatically only
  when a tagged GitHub Release is published; non-release runs are manual and never publish.
- Migrated add-on builds from the deprecated monolithic builder to Home Assistant's composable
  build actions.
- Clarified fork authorship and upstream attribution, retained `serjeleone` as repository
  maintainer, and removed the inherited sponsorship configuration.
