...
fn resolve_app_version(arg: Option<&str>) -> String {
    if let Some(version) = arg.map(str::trim).filter(|value| !value.is_empty()) {
        return version.to_string();
    }
    if let Ok(Some(version)) = opt_env_var::<String>("GOVEE_APP_VERSION") {
...
static ACTIVE_APP_VERSION: once_cell::sync::Lazy<arc_swap::ArcSwap<String>> =
    once_cell::sync::Lazy::new(|| {
        arc_swap::ArcSwap::new(std::sync::Arc::new(DEFAULT_APP_VERSION.to_string()))
    });
...
fn user_agent(version: &str) -> String {
    format!("GoveeHome/{version} (com.ihoment.GoVeeSensor; build:8; iOS 26.5.0) Alamofire/5.11.0")
}

fn app_version_hint(body_bytes: &[u8], version: &str) -> Option<String> {
    let raw: serde_json::Value = serde_json::from_slice(body_bytes).ok()?;
    let message = raw.get("message").and_then(|value| value.as_str())?;
    if message.contains("app version is too low") || message.contains("upgrade the version") {
        Some(format!(
            "Govee rejected app version {version}. Set --govee-app-version or \
             GOVEE_APP_VERSION to the current Govee Home version and restart."
        ))
    } else {
        None
    }
...
pub struct UndocApiArguments {
    ...
    /// Override the Govee Home app version sent to the undocumented API.
    /// Defaults to GOVEE_APP_VERSION and then the bundled fallback.
    #[arg(long, global = true)]
...
impl UndocApiArguments {
    ...
    pub fn api_client(&self) -> anyhow::Result<GoveeUndocumentedApi> {
        ...
        let app_version = resolve_app_version(self.govee_app_version.as_deref());
        ACTIVE_APP_VERSION.store(std::sync::Arc::new(app_version));
...
impl GoveeUndocumentedApi {
        ...
        cache_get(
            ...
            async {
                let response = reqwest::Client::builder()
                    ...
                    .header("User-Agent", user_agent(active_app_version().as_str()))
                    .send()
                    .await?;
    ...
    async fn login_account_impl(&self) -> anyhow::Result<CacheComputeResult<LoginAccountResponse>> {
        ...
        if let Some(hint) = app_version_hint(&body_bytes, active_app_version().as_str()) {
            return Err(NoCacheError(anyhow::anyhow!(hint)).into());
        }
